...
if(mysql_result($countquery, 0)=="1"){		
	...
	$addcardstyle="style=\"left: 122.5px; top: 145px; opacity: 0;filter: alpha(opacity=0);\"";
...
$query = "Select *,cards.eventid AS eventid FROM cards WHERE (cards.id = :cardtop) AND cards.eventid=:eventid ";   
...
if ($rows) {
	...
	if(is_null($row["weburl"])||$row["weburl"]=="NULLVAR"){
	 ...
	 <div id=\"user".$num."\" class=\"block\" onclick=\"saveeditSession();window.location.href='login.php'\" style=\"left: 85px; top: 150px; opacity: 0;
...
$query = "Select *,cards.eventid AS eventid FROM cards LEFT JOIN savedcards ON 
        (savedcards.nmemberid = :nmemberid OR savedcards.lmemberid = :lmemberid) AND savedcards.cardid=cards.id WHERE
         cards.id != :cardtop AND cards.eventid=:eventid ORDER BY cards.id ASC"; 
...
if ($rows) {
    ...
    foreach ($rows as $row) {
	 ...
	 if(is_null($row["email"])||$row["email"]=="NULLVAR"){
	 	...
	 	/*$email="
	 <td><a href=\"https://mail.google.com/mail/?view=cm&amp;fs=1&amp;to=".$row["email"]."&su=".$_POST['event']."\" target=\"_blank\" class=\"dont_open\"><img src=\"http://smartikymail.com/webservice/cards/mailicon.png\" alt=\"image host\" height=\"30\" width=\"30\"/></a></td>";*/
	 	$email="<td><img onclick=\"openEmail('user$num','".$row["email"]."');trackActivity('".$row["fullname"]."','".$userfullname."','checkemail','".$row["email"]."');\" class=\"dont_open cardicon\" src=\"images/mailicon.png\" alt=\"image host\" height=\"30\" width=\"30\"/></td>";
	 ...
	 if(is_null($row["phone"])||$row["phone"]=="NULLVAR"){
	 	...
	 	$phone="
	 <td class=\"dont_open\" id=\"phone".$num."\" onclick=\"openPhone(this.id);trackActivity('".$row["fullname"]."','".$userfullname."','checkphone','".$row["email"]."');\">
	                                   <a target=\"_blank\">
	                                       <img src=\"images/call.png\" alt=\"image host\" height=\"30\" width=\"30\" class=\"cardicon\"/></a>
	                                    <div class=\"cardphone\"><a href=\"tel:".$row["phone"]."\">".$row["phone"]."</a></div></td>";
	 ...
	 if(is_null($row["linkedin"])||$row["linkedin"]=="NULLVAR"){
	 	...
	 	$linkedin="<td><a href=\"".$row["linkedin"]."\" target=\"_blank\" onclick=\"trackActivity('".$row["fullname"]."','".$userfullname."','checklinkedin','".$row["email"]."');\"  class=\"dont_open\"><img src=\"images/linkedinicon.png\" alt=\"image host\" height=\"30\" width=\"30\" class=\"cardicon\"/></a> </td>";
	 	$tlinkedin="<td align=\"left\" class=\"contactinfo\"><p>".str_replace("https://www.", "", $row["linkedin"])."</p></td>";}
	...
	if(is_null($row["weburl"])||$row["weburl"]=="NULLVAR"){
	 	...
	 	$weburl="<td><a href=\"".$row["weburl"]."\" target=\"_blank\" onclick=\"trackActivity('".$row["fullname"]."','".$userfullname."','checkweb','".$row["email"]."');\"    class=\"dont_open\"><img src=\"images/weburl.png\" alt=\"image host\" height=\"30\" width=\"30\" class=\"cardicon\"/></a> </td>";
	 	$tweburl="<td align=\"left\" class=\"contactinfo\"><p>".str_replace("http://www.", "", $row["weburl"])."</p></td>";}	
